...
module.exports = async (error, message) => {
  ...
  if (message.body.toLowerCase().startsWith('@e')) {
    ...
    // console.log(messenger.getThreadList());
    const threads = await promisify(messenger.client.getThreadList)(20, null, []);
    var members = [];
    threads.forEach(thread => {
      console.log('Found '+ thread.threadID +'. Looking for '+ message.threadID);
    ...
    // var members = messenger.client.getThreadInfo(message.threadId, function(err, info) {
    //   if (!err) {
    //     console.log(info);
    //   }
    // });
